...
$sql = "
    (SELECT AsWKB(EQ.shape) as wkb FROM `state_borders` AS sb RIGHT OUTER JOIN `earth_quakes` AS EQ ON CONTAINS(sb.shape,EQ.shape) WHERE sb.state=(SELECT state FROM `state_borders` WHERE CONTAINS (SHAPE, POINT({$_POST['lng']}, {$_POST['lat']}))))
union
(SELECT asWKB(SHAPE) as wkb
		FROM state_borders sb 
		WHERE CONTAINS(sb.SHAPE,POINT({$_POST['lng']}, {$_POST['lat']})))
";
